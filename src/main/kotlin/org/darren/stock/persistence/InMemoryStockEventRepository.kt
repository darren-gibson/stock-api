...
class InMemoryStockEventRepository: StockEventRepository {
    ...
    override fun getEvents(location: String, product: String): Iterable<StockPotEvent> {
        val eventsInSequenceTheyOccurred = events[Pair(location, product)] ?: emptyList()
        return eventsInSequenceTheyOccurred.mapIndexed { i, e -> Pair(Pair(i, e.eventDateTime), e) }
            .sortedWith(compareBy({ it.first.second }, { it.first.first })).map { it.second }
    }

    override fun insert(location: String, product: String, event: StockPotEvent) {
