package com.darren.stock.domain.handlers

import com.darren.stock.domain.InsufficientStockException
import com.darren.stock.domain.MoveResult
import com.darren.stock.domain.OperationNotSupportedException
import com.darren.stock.domain.StockMovement
import com.darren.stock.domain.actors.TrackedStockPotMessages
import kotlinx.coroutines.CompletableDeferred
import java.time.LocalDateTime

class MoveHandler(private val helper: HandlerHelper) {
    suspend fun move(movement: StockMovement) {
        val from = helper.getStockPot(movement.from, movement.product)
        val to = helper.getStockPot(movement.to, movement.product)

        if (from is ChannelType.TrackedChannel && to is ChannelType.TrackedChannel) {
            move(movement, from, to)
        } else {
            throw OperationNotSupportedException("both ${movement.from} and ${movement.to} must be Tracked locations.")
        }
    }

    private suspend fun move(move: StockMovement, from: ChannelType.TrackedChannel, to: ChannelType.TrackedChannel) {
        val result = CompletableDeferred<MoveResult>()

        with(move) {
            from.channel.send(TrackedStockPotMessages.MoveEvent(product, quantity, to.channel, reason, LocalDateTime.now(), result))
            when (result.await()) {
                MoveResult.Success -> return
                MoveResult.InsufficientStock -> throw InsufficientStockException()
            }
        }
    }
}